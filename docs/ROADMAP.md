...
- **Read-only ground-source connectors:** Jira sync (read-only) ✓ · **Confluence fetch-by-page-id (read-only) ✓** (`GET /v1/confluence/:id`, storage-format→text, same never-writes guard as Jira) · observability (Datadog/Grafana/Sentry/Splunk) — ACL-mirrored, PII-sanitized. **Unblocks** Wave 3.
...
- **WriteGate** *(primitive + first real target shipped ✓):* `plan → named approval → apply → verify → append-only audit`. Read-only default; the only path Arbiter ever writes. Refuses to apply without a named human approval, and **HARD-refuses the connected Jira workspace at both register and apply time** (non-negotiable). Ships with the in-memory `SandboxWriteTarget` **and a real `GitHubWriteTarget`** (creates issues/comments in one configured repo via `Bearer` auth, `verify` re-GETs; `writeTargetFor(config)` picks GitHub-vs-sandbox). Proven in `tests/writegate.test.ts` + `tests/github.test.ts` (write path, verify, factory, gate flow, Jira-id refusal). Additional targets, if needed, use **free/OSS** systems (GitLab, Gitea) via the same `WriteTarget` interface — **no paid SaaS** (TestRail/Xray are out per the no-paid-services rule).
...
| ~~**pgvector / dense retrieval**~~ ✅ shipped | Dense retrieval behind the same `retrieveKnowledge` seam: **free, offline, local embeddings** (`@huggingface/transformers`, all-MiniLM-L6-v2, 384-d — no paid API) + pgvector cosine search (`0010`, ivfflat). Opt-in via `ARBITER_EMBEDDINGS=local`; TF-IDF stays the default so the offline path and CI are untouched. Verified end-to-end (a lexically-unrelated query surfaces the right chunk); `tests/embeddings.test.ts` covers ranking + the seam model-free | — (swap ivfflat→hnsw for scale) |
...
| **Gated Defect Write-Back → GitHub** (Wave 1 #3) | `GitHubWriteTarget` shipped; the gated plan→approve→apply UI is the remaining piece | **only** against GitHub / a sandbox Jira / free-OSS trackers, with explicit per-target authorization — never the connected workspace, never a paid SaaS |
