...
**Substrate & hardening (all shipped):** per-project **RAG** knowledge store + retrieval, **WriteGate** (gated writes), **Quality Metrics** layer, the **output PII re-scan gate**, an independent-judge **Eval Workbench** core + **LiteLLM/OpenAI-compatible** 2nd provider, a real **OTLP** exporter, tenant-scoped de-masking, locale + server-side-Presidio recognizers, and provider **streaming**. The deferred table in the roadmap is fully worked through; only external-service wiring (live collector/gateway/embeddings) and the external Python eval tools (Ragas/garak/PyRIT) remain as documented drop-ins.

The roadmap was **re-planned around the QA→QE gap analysis** into value-ordered **Waves** — see **[`docs/ROADMAP.md`](docs/ROADMAP.md)** and **[`docs/QA-GAP-ANALYSIS.md`](docs/QA-GAP-ANALYSIS.md)**. The 26 shipped workflows complete the value-ordered wave plan. Remaining work is **substrate/hardening** (tracked in `docs/ROADMAP.md`): the RAG knowledge substrate, WriteGate (gated writes to sandbox targets), an output PII re-scan gate (unblocks Synthetic Test-Data), real OTLP→Langfuse export, LiteLLM judge independence, and the full LLM Eval Workbench — plus the lower-ROI Wave-6 authoring tail (persona/mobile/mutation/chaos/DR/SRE/estimation drafters).
